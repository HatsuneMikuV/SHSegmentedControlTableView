...
class ThirteenViewController: SHBaseViewController, SHSegTableViewDelegate {
    
    var segTableView:SHSegmentedControlTableView!
    
    var segmentControl:SHSegmentControl!
    
    var headerView:SHHeaderView!
    ...
    func segTableViewDidScrollY(_ offsetY: CGFloat) {
        ...
        if offsetY <= 0 {
            self.headerView?.getScaleBlock(scaleH: 0 - offsetY)
        }
    }
    ...
    func getHeaderView() -> SHHeaderView {
        if self.headerView != nil {
            return self.headerView
        }
        let header:SHHeaderView = SHHeaderView.init(frame: CGRect.init(x: 0, y: 0, width: SCREEN_WIDTH, height: 200))
